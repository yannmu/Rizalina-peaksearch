...
ibinning = 1
script, job_name = create_job_name(script)
for ids in datasets:
	for it in range(1):
		log_time =  str(time.time())
		command = "qsub -q gerda -V -d /nfs/gerda2/users/rizalinko/gamma-fitter/logs3/  -e localhost:erlog_{}.txt   -o localhost:log_{}.txt -N excess_limit ".format(log_time, log_time, job_name, it)
