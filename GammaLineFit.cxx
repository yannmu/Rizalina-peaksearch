...
namespace std {
    ...
    void GammaLineFit::ResetFit(TString name)
    {
		  //! create fit function
        ...
        double f2 = 0.;
        if(fPeakPos.size() == 1){
			TF1 *f2 = new TF1("linfit", "[0]+[1]*x", fRange.first, fRange.second);
			...
			TF1 *fl1 = new TF1("linfit1", "[0]+[1]*x", fRange.first, fPeakPos.at(1)-3);
			TF1 *fl2 = new TF1("linfit2", "[0]+[1]*x", fPeakPos.at(1)+3, fRange.second );
			double cndf1 = 0.;
			double cndf2 = 0.;
			...
			if (fRange.second - fPeakPos.at(1) > 4){
			 ...
			 if (cndf1 > 1.1 || cndf1 < 0.5){
				 ...
				 if (cndf2 > 1.1 || cndf2 < 0.5){
					std::cout << "both fits are screwed" << std::endl;
					exit(0);
					f = maxBkg;
					s = 0;				 
				 }
    ...
    void GammaLineFit::Fit(TString name)
    {
...
//sprintf("%s.txt", name);
    ...
    */
    ...
    {	
		...
		/*std::cout << "check marginalised" << std::endl;
		vector<string> marhists(5);
		marhists[0] = "intnsty0";
		marhists[1] = "bkgrnd";
		marhists[2] = "mean0";
		marhists[3] = "fwhm0";
		...
		std::cout << "*******************************************" << endl;
		for (int npar  = 0; npar<4;npar++){
			std::cout << marhists[npar] << endl;
			BCH1D* mar = fHistFitter->GetMarginalized(Form(marhists[npar].c_str()));
			double mode = mar->GetMode();
			double quantile90 = mar->GetQuantile(0.9);
			std::cout << "mode:" << "\t" << mode << endl;
			std::cout << "mean:" << "\t" << mar->GetMean() << endl;
			std::cout << "STD:" << "\t" << mar->GetSTD() << endl;
			double low, high;
			mar->GetSmallestInterval(low,high);
			std::cout << "68% smallest interval:" << "\t" << "["<<
					low << ";" << high << "]" << endl;

			mar->GetSmallestInterval(low,high,0.955);
			std::cout << "95% smallest interval:" << "\t" << "["<<
					low << ";" << high << "]" << endl;


			mar->GetSmallestInterval(low,high, 0.997);
			std::cout << "97% smallest interval:" << "\t" << "["<<
					low << ";" << high << "]" << endl;

			std::cout <<"bin center array" <<endl;
			TH1D * h_mar = mar->GetHistogram();
			int nentries = h_mar->GetEntries();
			/*for(int i =0; i<nentries; i++){
				std::cout << h_mar->GetBinCenter(i) << "\t";
			}
			std::cout <<endl;
			for(int i =0; i<nentries; i++){
				std::cout << h_mar->GetBinContent(i) << "\t";
			}
			std::cout<<endl;
