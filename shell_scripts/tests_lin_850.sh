...
ds=$i
echo $ds
for ((en=847; en<850; en+=50));
do
odir="/nfs/gerda2/users/rizalinko/gamma-fitter/run0053-run0092/sensitivity_lin_fit_test/"
if [ ! -d ${odir} ]; then
    mkdir -m g=wr ${odir}
fi
...
time /nfs/gerda2/users/rizalinko/gamma-analysis/peaksearch -g ${line} -e ${en} -k ${kl} -d ${ds} -b ${bing} -r ${rescrv} -o ${odir} -j ${batprec} -w ${woi} -f ${pseudo_path} -i ${pseudo_iter} -t 
