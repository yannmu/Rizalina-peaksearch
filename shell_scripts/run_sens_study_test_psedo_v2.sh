...
odir="/nfs/gerda2/users/rizalinko/gamma-fitter/test_pseudo/"
if [ ! -d ${odir} ]; then
    mkdir -m g=wr ${odir}
fi
...
gammacounts -g ${line} -e ${en} -k ${kl} -d ${ds} -b ${bing} -r ${rescrv} -o ${odir} -j ${batprec} -w ${woi} -f ${pseudo_path} -i ${pseudo_iter} -t > log_${log_file}.txt > out.txt
#break
