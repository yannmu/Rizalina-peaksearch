...
glines="${bin_dir}/g_lines_strong.txt"
...
do
odir="/nfs/gerda2/users/rizalinko/gamma-fitter/run0053-run0092/limit_lin_fit_v1"
    mkdir -m g=wr ${odir}
fi
...
${bin_dir}/peaksearch -g ${line} -e ${en} -k ${kl} -d ${ds} -b ${bing} -r ${rescrv} -o ${odir} -j ${batprec} -w ${woi} -s ${sfile} -n ${glines}  > log_${log_file}.txt
